...
public class Juego {
    ...
    public void iniciaImprimeMyr(Contenedor[] contenedores){
        Contenedor.mostrarMyrInsertados(contenedores,nivelActual, jugadores);
    }
    ...
    public void muestraGanadores(){
        ...
        if (myr==0){
            ...
            iGraficaJuego.getLResultadosFinales(1).setText("Perdieron");
            iGraficaJuego.getLMuestraTiempoResResultados(1).setForeground(Color.white);
        }    
        else{
            iGraficaJuego.getLResultadosFinales(0).setText("GANARON: ");
            StringBuilder fraseGanadores=new StringBuilder("<html>");
            iGraficaJuego.getPResultFinal().setBackground(Color.CYAN);
            for (Jugador jugador:jugadores)
                if (jugador.getNoPuntos()==myr)
                    fraseGanadores.append("¡¡ "+jugador.getNombreJug()+" !!<br>");
