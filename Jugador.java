public class Jugador {
    ...
    public void clasificarDesecho(Desechos desecho, Contenedor contenedor, Nivel niv, Historial hist){
        if (contenedor.verificarDesecho(desecho)){
            System.out.println("\nDesecho clasificado correctamente: +"+niv.getPuntosRespCorrecta()+" pts.");
            hist.escribirTxt("Correcto! +"+niv.getPuntosRespCorrecta()+" pts.\n");
            contenedor.agregarDesecho(desecho);
            ganarPuntos(niv);
        }     
        else{
            System.out.println("\nDesecho clasificado incorrectamente: -"+niv.getVidasRespIncorrecta()+" vidas.");
            hist.escribirTxt("Incorrecto, -"+niv.getVidasRespIncorrecta()+" vidas.\n");
            perderVidas(niv);
        }        
    }
